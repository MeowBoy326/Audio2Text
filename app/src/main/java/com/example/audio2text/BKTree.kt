...
class BKTree( private val distanceFunc : (String,String) -> Int ) {
    ...
    fun getSpellSuggestion( word: String , tolerance : Int = 1 ) : List<String> {
        ...
        if( root != null ) {
            val suggestion = getSpellSuggestion(root!!,
                word.replaceFirstChar { it.lowercase(Locale.ROOT) }, tolerance)
            if( suggestion.size == 1 && suggestion[0].replaceFirstChar { it.lowercase(Locale.ROOT) }
                    .equals(word) )
                return listOf()
            else
                return suggestion
