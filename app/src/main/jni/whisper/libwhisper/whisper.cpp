...
static bool whisper_model_load(struct whisper_model_loader *loader, whisper_context &wctx) {
...

    {
        ...
        {
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // mlp_ln_w
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // mlp_ln_b

            ctx_size += n_audio_layer * (4 * n_audio_state * n_audio_state *
                                         ggml_type_sizef(wtype));         // mlp_0_w
            ctx_size +=
                    n_audio_layer * (4 * n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // mlp_0_b
            ...
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_ln_0_w
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_ln_0_b

            ctx_size += n_audio_layer * (n_audio_state * n_audio_state *
                                         ggml_type_sizef(wtype));         // attn_q_w
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_q_b

            ctx_size += n_audio_layer *
                        (n_audio_state * n_audio_state * ggml_type_sizef(wtype)); // attn_k_w

            ctx_size += n_audio_layer * (n_audio_state * n_audio_state *
                                         ggml_type_sizef(wtype));         // attn_v_w
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_v_b

            ctx_size += n_audio_layer * (n_audio_state * n_audio_state *
                                         ggml_type_sizef(wtype));         // attn_ln_1_w
            ctx_size +=
                    n_audio_layer * (n_audio_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_ln_1_b
        }
        ...
        // decoder layers
        {
            ...
            ctx_size += n_text_layer * (4 * n_text_state * n_text_state *
                                        ggml_type_sizef(wtype));         // mlp_0_w
            ctx_size +=
                    n_text_layer * (4 * n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // mlp_0_b
            ...
            ctx_size +=
                    n_text_layer * (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_ln_0_w
            ctx_size +=
                    n_text_layer * (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_ln_0_b

            ctx_size += n_text_layer *
                        (n_text_state * n_text_state * ggml_type_sizef(wtype));         // attn_q_w
            ctx_size += n_text_layer * (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_q_b

            ctx_size += n_text_layer *
                        (n_text_state * n_text_state * ggml_type_sizef(wtype)); // attn_k_w

            ctx_size += n_text_layer *
                        (n_text_state * n_text_state * ggml_type_sizef(wtype));         // attn_v_w
            ctx_size += n_text_layer * (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_v_b

            ctx_size += n_text_layer * (n_text_state * n_text_state *
                                        ggml_type_sizef(wtype));         // attn_ln_1_w
            ctx_size +=
                    n_text_layer * (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // attn_ln_1_b
            //
            ctx_size += n_text_layer *
            ...
            ctx_size += n_text_layer *
                        (n_text_state * n_text_state * ggml_type_sizef(wtype)); // cross_attn_k_w
...
static bool whisper_encode_internal(
    ...
    if (!use_coreml && !use_openvino) {
        ...
        struct ggml_tensor *e_pe = ggml_view_2d(ctx0, model.e_pe, model.e_pe->ne[0], n_ctx,
                                                e_pe_stride, e_pe_offset);
    ...
    // pre-compute cross-attention memory
    {
        ...
        for (int il = 0; il < model.hparams.n_text_layer; ++il) {
            ...
            Kcross = ggml_scale_inplace(ctx0, Kcross,
...
static bool whisper_decode_internal(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            Qcur = ggml_scale_inplace(ctx0, Qcur,
                                      ggml_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));
            ...
            Kcur = ggml_scale_inplace(ctx0, Kcur,
                                      ggml_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor *k = ggml_view_1d(ctx0, kv_self.k, N * n_state,
            ...
            struct ggml_tensor *Kcross =
                    ggml_reshape_3d(ctx0,
                                    ggml_view_1d(ctx0, wstate.kv_cross.k, M * n_state,
                                                 il * M * ggml_element_size(wstate.kv_cross.k) *
                                                 n_state),
...
static void
log_mel_spectrogram_worker_thread(int ith, const std::vector<float> &hann, const float *samples,
...
static bool log_mel_spectrogram(
...

    {
        ...
        // main thread
        log_mel_spectrogram_worker_thread(0, hann, samples, n_samples, fft_size, fft_step,
                                          n_threads, filters, speed_up, mel);
...
struct whisper_state *whisper_init_state(whisper_context *ctx) {
    ...
    if (!kv_cache_init(ctx->model.hparams, scale * MEM_REQ_KV_SELF.at(ctx->model.type),
                       state->decoders[0].kv_self, ctx->itype, ctx->model.hparams.n_text_ctx)) {
    ...
    if (!kv_cache_init(ctx->model.hparams, scale * MEM_REQ_KV_CROSS.at(ctx->model.type),
    ...
    state->buf_compute.resize(scale * std::max(MEM_REQ_ENCODE.at(ctx->model.type),
...
int whisper_pcm_to_mel_with_state(struct whisper_context *ctx, struct whisper_state *state,
    ...
    if (!log_mel_spectrogram(*state, samples, n_samples, WHISPER_SAMPLE_RATE, WHISPER_N_FFT,
                             WHISPER_HOP_LENGTH, WHISPER_N_MEL, n_threads, ctx->model.filters,
                             false, state->mel)) {
...
int whisper_pcm_to_mel_phase_vocoder_with_state(struct whisper_context *ctx,
    ...
    if (!log_mel_spectrogram(*state, samples, n_samples, WHISPER_SAMPLE_RATE, 2 * WHISPER_N_FFT,
                             2 * WHISPER_HOP_LENGTH, WHISPER_N_MEL, n_threads, ctx->model.filters,
                             true, state->mel)) {
        log("%s: failed to compute mel spectrogram\n", __func__);
        return -1;
    }
...
int whisper_decode_with_state(struct whisper_context *ctx, struct whisper_state *state,
    ...
    if (!whisper_decode_internal(*ctx, *state, state->decoders[selected_decoder_id], tokens,
...
whisper_decode(struct whisper_context *ctx, const whisper_token *tokens, int n_tokens, int n_past,
    ...
    if (!whisper_decode_internal(*ctx, *ctx->state, ctx->state->decoders[selected_decoder_id],
...
static void whisper_process_logits(
    ...
    auto &logprobs = decoder.logprobs;
    {
        ...
        memcpy(logits.data(), state.logits.data() + (state.logits.size() - n_logits),
...
int whisper_full_with_state(
    ...
    if (ctx->isStopped) {
        if (params.inferenceStoppedCallback) {
            params.inferenceStoppedCallback(ctx, ctx->state,
                                            params.inferenceStoppedCallback_user_data);
        }
        //whisper_free(ctx);
        return 1;
    } else {
    ...
    if (params.speed_up) {
        if (whisper_pcm_to_mel_phase_vocoder_with_state(ctx, state, samples, n_samples,
                                                        params.n_threads) != 0) {
    ...
    if (params.language == nullptr || strlen(params.language) == 0 ||
        ...
        if (params.ignore_lang_id != nullptr &&
            strcmp(params.ignore_lang_id, params.language) == 0) {
            return 2;
        }
    ...
    // prepare prompt
    {
        ...
        if (!params.prompt_tokens && params.initial_prompt) {
            ...
            prompt_tokens.resize(whisper_tokenize(ctx, params.initial_prompt, prompt_tokens.data(),
                                                  prompt_tokens.size()));
    ...
    while (true) {
        ...
        for (int it = 0; it < (int) temperatures.size(); ++it) {
            ...
            // run whisper_decoder() only for decoder 0 and copy the results for the other decoders
            {
                ...
                if (!whisper_decode_internal(*ctx, *state, state->decoders[0], prompt.data(),
                                             prompt.size(), 0, params.n_threads)) {
                    log("%s: failed to decode\n", __func__);
                    return -7;
                }
            ...
            for (int i = 0, n_max = whisper_n_text_ctx(ctx) / 2 - 4; i < n_max; ++i) {
                ...
                for (int j = 0; j < n_decoders_cur; ++j) {
                    ...
                    switch (params.strategy) {
                        ...
                        case whisper_sampling_strategy::WHISPER_SAMPLING_BEAM_SEARCH: {
                            ...
                            const auto tokens_new = whisper_sample_token_topk(*ctx, *state, decoder,
                ...
                if (params.strategy == whisper_sampling_strategy::WHISPER_SAMPLING_BEAM_SEARCH) {
                    ...
                    for (int j = 0; j < n_decoders_cur; ++j) {
                        ...
                        while (beam_candidates.size() > cur_c &&
                               ...
                               cur.sequence.sum_logprobs_all && i > 0) {
                            ++cur_c;
                        }
                ...
                for (int j = 0; j < n_decoders_cur; ++j) {
...

                    {
                        ...
                        if (token.id == whisper_token_eot(ctx) ||               // end of text token
                            ...
                            if (ctx->result_len > 0) {
                                __android_log_print(ANDROID_LOG_INFO, "Decoder info",
                                                    "Previous result_len : %d", ctx->result_len);
                                decoder.sequence.tokens.resize(ctx->result_len);
                            }

                            ctx->seek_delta = seek_delta;
                            ctx->result_len = result_len;
                            __android_log_print(ANDROID_LOG_INFO, "Decoder info",
                                                "Reach result_len : %d", result_len);
                ...
                for (int j = 0; j < n_decoders_cur; ++j) {
                    ...
                    if (!whisper_decode_internal(*ctx, *state, decoder, decoder.tokens_tmp.data(),
                        ...
                        if (!text.empty()) {
                            ...
                            if (params.print_realtime) {
                                if (params.print_timestamps) {
                                    __android_log_print( ANDROID_LOG_INFO,"With timestamps","[%s --> %s]  %s\n", to_timestamp(tt0).c_str(),
